...
//修复标记：设置时间
	...
	public List<Goods> findAll() throws Exception{
		...
		String sql = "SELECT name,num,content,type_id,image,producter_id,price from goods" ;
		this.pstmt = this.conn.prepareStatement(sql) ;
		ResultSet rs = this.pstmt.executeQuery() ;
		Goods good = null ;
		while(rs.next()){
	...
	public List<Goods> findAllNotAuditing() throws Exception{
		List<Goods> all = new ArrayList<Goods>() ;
		String sql = "SELECT id,num,content,type_id,image,producter_id,price,name from goods where status=1" ;
		this.pstmt = this.conn.prepareStatement(sql) ;
		ResultSet rs = this.pstmt.executeQuery() ;
		Goods good = null ;
		while(rs.next()){
	...
	public boolean doUpdate(Goods good) throws Exception{
		...
		//标记：修复这里的时间
		pstmt.setString(10,"2015-12-11 08:08:08");
